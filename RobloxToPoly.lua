print("Welcome to ROBLOX to Polytoria (v0.0.6).\nThis is a Luau -> Polytoria Lua converter.\nInput file name:")
...
local res, err = pcall(function()
    ...
    if selection == "START" then
        ...
        for line, nm in io.lines(filename) do
            ...
            line = line:gsub("%W\\.RenderStepped", "game.Rendered")
            line = line:gsub("'RemoteEvent'", "'Event'")
            line = line:gsub("'UserInputService'", "'Input'")
            line = line:gsub("%W\\.CFrame = CFrame.lookAt(%W,.%W,.%W)", "%1:LookAt(%1, %2, %3)")
